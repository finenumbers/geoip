...
Portainer использует файл [`infra/portainer/stack.compose.yml`](../infra/portainer/stack.compose.yml), который подключает два корневых compose-файла:

```
infra/portainer/stack.compose.yml
    ├── docker-compose.yml        ← базовые сервисы
    └── docker-compose.prod.yml   ← production: только :8080, лимиты, backup
```
...
Portainer сам клонирует репозиторий, собирает образы и поднимает контейнеры.
...
**Первый деплой занимает 5–15 минут** (зависит от CPU и сети — скачиваются базовые образы и собирается Node).
...
| Причина | Решение |
|---------|---------|
| Неверный Compose path | Должен быть **`infra/portainer/stack.compose.yml`** |
