...
Cron: configurable (Admin → Общие; default 06:00 в displayTimezone). Browse: `/browse/rir`, API: `GET /api/v1/table/rir`.  
**Не джойнится** с GeoIP city/country; `/ready` GeoIP не зависит от RIR. Поле `registry` — источник делегирования (в т.ч. `iana`). Семантика `cc` — legal country of holder, не геолокация.
