...
- (Для NPM в Docker) внешняя сеть **`proxy`**: `docker network create proxy` — если ещё нет; `geoip_api` подключается к ней в Portainer/prod compose
...
| `api` | `geoip_api` | HTTP API, migrations при старте; в prod также в сети `proxy` (NPM) |
